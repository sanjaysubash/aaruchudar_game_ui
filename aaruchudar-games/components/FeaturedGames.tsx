  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/GamesCode" className="text-xs px-3 py-1.5 rounded-lg bg-white/10 border border-white/20 hover:bg-white/15 transition-colors">View All</Link>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {featured.map((game) => (
          <Link
            key={game.slug}
            href={`/GamesCode/${game.slug}`}
